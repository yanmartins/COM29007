...
for SNR = 0 : SNR_max
    ...
    info_rx1_Eb_No = awgn(info_tx1, 10^(Eb_No/10) - (10*log10(A1))); % Gera o ruído branco em função da variância SNR
    info_rx2_Eb_No = awgn(info_tx2, 10^(Eb_No/10) - (10*log10(2*A1)));
    ...
    info_rx_filter1_Eb_No = filter(filtro_rx, 1, info_rx1_Eb_No)/N;
    info_hat1 = info_rx_filter1_Eb_No(N : N : end) > limiar1; % Informação estimada
    
    info_rx_filter2_Eb_No = filter(filtro_rx, 1, info_rx2_Eb_No)/N;
    info_hat2 = info_rx_filter2_Eb_No(N : N : end) > limiar2; % Informação estimada
    
    % Probabilidade de erro de bit do sinal Rx
    Pb1_SNR(SNR+1) = sum(xor(info,info_hat1))/length(info_hat1);
    Pb2_SNR(SNR+1) = sum(xor(info,info_hat2))/length(info_hat2);
    
    Pb1_Eb_No(Eb_No+1) = qfunc(sqrt(10^(Eb_No/10)));
    Pb2_Eb_No(Eb_No+1) = qfunc(sqrt(2*10^(Eb_No/10)));
end

% Plotando os resultados
figure, 
subplot(121) 
semilogy([0:SNR_max],Pb1_SNR); hold on;
semilogy([0:SNR_max],Pb2_SNR);
...
subplot(122) 
semilogy([0:SNR_max],Pb1_Eb_No); hold on;
semilogy([0:SNR_max],Pb2_Eb_No);
title('Simulação teórica');
ylim([10^-3 10^0]);
xlabel('Eb_No');
ylabel('Probabilidade de erro (Pb)');
legend('Sinalização NRZ unipolar','Sinalização NRZ bipolar');
